  ...
  <div class="chartWrap">
    <div class="chartWrap-title-tools">
      <el-select v-model="query.type" placeholder="请选择" style="width:100px" @change="fetchData">
        <el-option label="有档人员" value="1"></el-option>
        <el-option label="库存正副本" value="2"></el-option>
      </el-select>
      <span
    ...
    <h3 class="chartWrap-title">库存统计</h3>
...
import { getInventoryData } from '@/api/chart'
      ...
      getInventoryData({
        ...
        if (type == '1') {
          ...
          res.response.list.forEach(item => {
            data.xAxis[0].data.push(new Date(item.startDate).format('yyyy-MM-dd'))
            // data.series[0].data.push(item.queDangCount)
            data.series[0].data.push(item.zaiKuCount || 0)
            data.series[1].data.push(item.diaoChuCount || 0)
            data.series[2].data.push(item.jieChuCount || 0)
          })
        } else {
          data.legend.data = ['正本数', '副本数', '正副本数']
          data.series = [
            {
              name: '正本数',
              type: 'bar',
          ...
          res.response.list.forEach(item => {
            ...
            data.series[0].data.push(item.isVolume)
            data.series[1].data.push(item.viceVolume)
            data.series[2].data.push(item.library)
          })
        }
        this.chartData = data
      })
